...
function carregarAvaliacoes(){
  try { return JSON.parse(localStorage.getItem('arteia_avaliacoes') || '[]') } catch { return [] }
}

function salvarAvaliacoes(lista){
  localStorage.setItem('arteia_avaliacoes', JSON.stringify(lista))
}
  ...
  return <section className="avaliacao-publica">
    ...
    <form onSubmit={enviar} className="avaliacaoForm">
      <div className="cols">
        ...
        <input placeholder="Sua profissão" value={form.profissao} onChange={e=>setForm({...form,profissao:e.target.value})}/>
      </div>
...
function Landing(){
  ...
  useEffect(()=>{
    async function carregarFinalizados(){
      const { count, error } = await supabase
        ...
        .in('status', ['pago','entregue','finalizado'])
      if(!error && typeof count === 'number') setFinalizados(BASE_CLIENTES_FINALIZADOS + count)
    }
    carregarFinalizados()
  },[])
  ...
  return <main className="landing">
    ...
    <section className="pagamentos-site">
      <h2>Formas de pagamento</h2>
      <div className="pagamentoGrid">{pagamentos.map(p=><div className="pagamentoItem" key={p.nome}><b>{p.nome}</b><span>{p.texto}</span></div>)}</div>
      <p style={{textAlign:'center',color:'#cbd5e1'}}>O cliente escolhe a forma de pagamento somente depois de aprovar a prévia.</p>
    </section>
    <section className="depoimentos">
      <h2>Clientes satisfeitos</h2>
      <div className="depoGrid">
        {depoimentos.map((d,i)=><div key={d.id || i} className="depoCard">⭐⭐⭐⭐⭐<p>"{d.texto}"</p><strong>{d.nome}</strong><small>{d.profissao}</small></div>)}
      </div>
    </section>
...
function Etapa2({ onConfirm }) {
  ...
  return <div className="formulario-etapa">
    <div className="etapa-header"><span className="numero-etapa">2 de 2</span><h2>Detalhes da Arte</h2><p>Faltam alguns detalhes</p></div>
    <form onSubmit={enviar} className="etapa-content">
      <h3>Arquivos</h3>
      <label className="upload">📷 Enviar foto para criar personagem/rosto<input type="file" accept="image/*" onChange={e=>{ setFoto(e.target.files[0]); setSemFotoAceito(false) }}/>{foto && <small>✓ {foto.name}</small>}</label>
      {!foto && <div className="aviso-sem-foto"><strong>⚠️ Você ainda não enviou foto.</strong><p>Sem foto, a arte será criada sem personagem e sem rosto personalizado.</p><label><input type="checkbox" checked={semFotoAceito} onChange={e=>setSemFotoAceito(e.target.checked)} /> Entendo e aceito seguir sem foto.</label></div>}
      ...
      <div className="cols"><input required placeholder="Profissão" value={form.profissao} onChange={e=>set('profissao',e.target.value)}/><input placeholder="Telefone que aparecerá" type="tel" inputMode="tel" value={form.telefone_arte} onChange={e=>set('telefone_arte',e.target.value)}/></div>
      ...
      <div className="resumo"><span>Resumo: <b>{form.tipo_arte}</b><br/><small>Pagamento será escolhido após aprovação da prévia.</small></span><strong>{money(tipo.preco)}</strong></div>
...
function Formulario() {
  ...
  return <main className="page formulario-page"><section className="hero"><div className="badge">🎨 Promova Profissional</div><h1>Faça seu pedido</h1></section><div className="card-formulario">{etapa === 1 && <Etapa1 tipo={tipo} setTipo={setTipo} setEtapa={setEtapa} />}{etapa === 2 && <Etapa2 onConfirm={() => setOk(true)} />}</div></main>
}
...
function AdminLogin({ onEntrar }){
  ...
  return <main className="page success"><h1>🔒 Painel Administrativo</h1><form onSubmit={tentar} className="card" style={{maxWidth:340,margin:'20px auto',textAlign:'left'}}><input required type="password" placeholder="Senha" autoFocus value={senha} onChange={e=>setSenha(e.target.value)} />{erro && <p className="erro">{erro}</p>}<button className="submit" type="submit">Entrar</button></form></main>
}
