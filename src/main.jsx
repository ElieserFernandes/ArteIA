...
const pagamentos = [
  { id:'PIX', titulo:'PIX', texto:'Pagamento rápido com confirmação pelo Mercado Pago.' },
  { id:'CARTAO_CREDITO', titulo:'Cartão de crédito', texto:'Pague pelo checkout seguro do Mercado Pago.' },
  { id:'CARTAO_DEBITO', titulo:'Cartão de débito', texto:'Use débito quando disponível no Mercado Pago.' },
  { id:'MERCADO_PAGO', titulo:'Mercado Pago', texto:'Escolha a melhor opção disponível no checkout.' }
]

function money(v){ return v.toLocaleString('pt-BR',{style:'currency',currency:'BRL'}) }

function App(){
  const [form,setForm]=useState({tipo_arte:'COMBO',forma_pagamento:'PIX',nome:'',whatsapp:'',cidade:'',estado:'',nome_arte:'',profissao:'',telefone_arte:'',cor_predominante:'',servicos:'',detalhes:''})
  ...
  const pagamento = pagamentos.find(p=>p.id===form.forma_pagamento)
  ...
  async function enviar(e){
    ...
    try{
      ...
      const { error:e3 } = await supabase.from('pagamentos').insert({
        pedido_id:pedido.id,
  ...
  return <main className="page">
    ...
    <form onSubmit={enviar} className="card">
      ...
      <div className="grid pagamentos">
        {pagamentos.map(p=><button type="button" key={p.id} onClick={()=>set('forma_pagamento',p.id)} className={'tipo pagamento '+(form.forma_pagamento===p.id?'ativo':'')}>
          <strong>{p.titulo}</strong><span>{p.texto}</span>
        </button>)}
      </div>
      <div className="aviso">O pagamento será feito pelo Mercado Pago. PIX, crédito, débito e carteira digital dependem das opções disponíveis no checkout.</div>
      ...
      <div className="resumo"><span>Resumo: <b>{form.tipo_arte}</b><br/><small>Pagamento: {pagamento.titulo}</small></span><strong>{money(tipo.preco)}</strong></div>
